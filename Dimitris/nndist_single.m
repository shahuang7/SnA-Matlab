function [ yVal, yInd, Batch ] = nndist_single( Batch, nD, nN, varargin )
%NNDIST Nearest-neighbor distances with single-precision input data
%   
%   [ yVal, yInd ] = nndist_single( Batch, nD, nN ) reads a total of nY data 
...
%   neighbors.
%
%   [ ... ] = nndist_single( Batch, nD, nN, 'batchOut', [ iBatch1, iBatch2 ] )
...
clk = clock;
fprintf( logID, 'NNDIST_SINGLE starting on %i/%i/%i %i:%i:%2.1f \n', ...
...
for iBatch = Options.batchOut( 1 ) : Options.batchOut( 2 )
    ...
    eval( [ xQStr, ' = double( ', xQStr, ' );' ] ); % convert to double 
    t = toc;
    ...
    for jBatch = jRead % Loop over the columns 
        ...
        eval( [ xTStr, ' = double( ', xTStr, ' );' ] ); % convert to double 
        t = toc;
        fprintf( logID, 'READ (%i,%i) %i %2.4f \n', ...
            iBatch, jBatch, nT, t );
...
fprintf( logID, 'NNDIST_SINGLE finished on %i/%i/%i %i:%i:%2.1f \n', ...
    clk( 1 ), clk( 2 ), clk( 3 ), clk( 4 ), clk( 5 ), clk( 6 ) );
fclose( logID );
