function [ yVal, yInd, Batch ] = nndist( Batch, nD, nN, varargin )
%NNDIST Nearest-neighbor distances
%   
%   [ yVal, yInd ] = nndist( Batch, nD, nN ) reads a total of nY data 
...
%   neighbors.
%
%   [ ... ] = nndist( Batch, nD, nN, 'batchOut', [ iBatch1, iBatch2 ] )
...
%   processed).
%   
%   [ ... ] = nndist( Batch, nD, nN, 'logFile', logFile ) dumps log
%   information in the file logFile. By default, log information is written
%   on screen.
%
%   [ yVal, yInd, Batch ] = sdmat( ... ) adds the field Batch( i ).gInd to 
...
%   single vectors.
%
%   nndist requires the MinMaxSelection MEXC routines for partial sorting
...
clk = clock;
fprintf( logID, 'NNDIST starting on %i/%i/%i %i:%i:%2.1f \n', ...
...
for iBatch = Options.batchOut( 1 ) : Options.batchOut( 2 )
    ...
    xQStr = [ 'xQ.', Batch( iBatch ).dataName ];  % string representing xQ     
    t = toc;
    fprintf( logID, 'READ (%i,%i) %i %2.4f \n', iBatch, iBatch, nQ, t );   
    
    tic
    evalStr = [ 'yNew = dmat( ', xQStr, ' );' ];
    eval( evalStr ); % Distance matrix for xQ
    t = toc;
    fprintf( logID, 'DMAT (%i,%i) %i %i %2.4f \n', ...
    ...
    for jBatch = jRead % Loop over the columns 
        ...
        xTStr = [ 'xT.', Batch( jBatch ).dataName ]; % string for xT
        t = toc;
        fprintf( logID, 'READ (%i,%i) %i %2.4f \n', ...
            iBatch, jBatch, nT, t );
            
        tic
        evalStr = [ 'yNew = dmat( ', xQStr, ',', xTStr, ' );' ];
...
fprintf( logID, 'NNDIST finished on %i/%i/%i %i:%i:%2.1f \n', ...
    clk( 1 ), clk( 2 ), clk( 3 ), clk( 4 ), clk( 5 ), clk( 6 ) );
fclose( logID );
