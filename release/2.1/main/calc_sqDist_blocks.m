function calc_sqDist_blocks(param,numWorker,workerID)
%
% calc_sqDist_blocks(param,numWorker,workerID)
%
...
% Copyright (c) Russell Fung 2014-2016
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  n = param.n;
  N = param.N;
  D = param.D;
  ...
  for row=myRows
    ...
    for col=get_row_col_range(param,numWorker,workerID,row);
      ...
      if (col>row)
        ...
        eval(command);
        dSq = zeros(n);
        dSq(1:j1-j0+1,1:i1-i0+1) = dmat(dataJ,dataI);
      else
        dSq = zeros(n);
        dSq(1:j1-j0+1,1:i1-i0+1) = dmat(dataJ);
      end
      write_dSq(fileName_template,io_format,1,row,col,dSq);
    end
  end
% end function calc_sqDist_blocks
