...
% data parameters
%
  param.rawDataFile = ['/state/partition2/test_data_with_masks.mat'];
  param.maskFile = ['/state/partition2/test_data_with_masks.mat'];
  %
...
%
  param.local_hostname = 'photon.phys.uwm.edu';
  param.local_destination = 'test_results/';
%%%%%
% remote cluster parameters
%   remote cluster is where you are sending the jobs to
%
  param.remote_hostname = 'watson.phys.uwm.edu';
  param.config = 'jobmanagerconfig1';
  param.username = 'russell';
  param.share_directory = '/share/apps/data/russell/TempFiles/';
  param.worker_directory_prefix = '/state/partition2/workerRF'; % on worker nodes
%%%%%
