function calc_dotProduct_blocks(param,numWorker,workerID)
%
...
% Parallelized 05th January 2016
  ...
  for row=myRows
    ...
    for col=get_row_col_range(param,numWorker,workerID,row);
      ...
      if (col>row)
        ...
        eval(command);
        dSq = zeros(n);
        dSq(1:j1-j0+1,1:i1-i0+1) = dpmat(dataJ,dataI);
      else
        dSq = zeros(n);
        dSq(1:j1-j0+1,1:i1-i0+1) = dpmat(dataJ);
      end
      write_dSq(fileName_template,io_format,1,row,col,dSq);
    end
  end
% end function calc_dotProduct_blocks
