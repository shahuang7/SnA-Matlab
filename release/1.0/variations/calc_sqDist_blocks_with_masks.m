function calc_sqDist_blocks_with_masks(param,numWorker,workerID)
%
  ...
  for row=myRows
    ...
    for col=get_row_col_range(param,numWorker,workerID,row);
      ...
      if (col>row)
        ...
        eval(command);
        dSq = zeros(n);
        dSq(1:j1-j0+1,1:i1-i0+1) = dmat_with_masks(dataJ.*maskJ,dataI.*maskI,maskJ,maskI);
      else
        dSq = zeros(n);
        dSq(1:j1-j0+1,1:i1-i0+1) = dmat_with_masks(dataJ.*maskJ,dataJ.*maskJ,maskJ,maskJ);
      end
      write_dSq(fileName_template,io_format,1,row,col,dSq);
    end
  end
% end function calc_sqDist_blocks_with_masks
