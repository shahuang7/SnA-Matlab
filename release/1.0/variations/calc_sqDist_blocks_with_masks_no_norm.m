function calc_sqDist_blocks_with_masks_no_norm(param,numWorker,workerID)
%
% calc_sqDist_blocks_with_masks_no_norm(param,numWorker,workerID)
%
% calculates the matrix of squared distances for a set of vectors, and
...
%
% Programmed 10th February 2014
% Comments and codes last updated March 2016
% Masks implemented May 2015
% Parallelized 14th December 2015
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  eval(command);
  command = sprintf('%s=%s'';',maskVar,maskVar);
  eval(command);
  ...
  for row=myRows
    ...
    for col=get_row_col_range(param,numWorker,workerID,row);
      ...
      if (col>row)
        ...
        eval(command);
        dSq = zeros(n);
        dSq(1:j1-j0+1,1:i1-i0+1) = dmat_with_masks_no_norm(dataJ.*maskJ,dataI.*maskI,maskJ,maskI);
      else
        dSq = zeros(n);
        dSq(1:j1-j0+1,1:i1-i0+1) = dmat_with_masks_no_norm(dataJ.*maskJ,dataJ.*maskJ,maskJ,maskJ);
      end
      write_dSq(fileName_template,io_format,1,row,col,dSq);
    end
  end
% end function calc_sqDist_blocks_with_masks_no_norm
